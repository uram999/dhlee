 ...
 * between thmd, and thm-qt and non-server tools.
*/
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), THM_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/thm/thm>";
    const std::string URL_WEBSITE = "<https://thmcore.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    // hathmoded $DATADIR/bootstrap.dat
...
/** Sanity checks
 *  Ensure that THM is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single THM process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", THM_CONF_FILENAME)).string());
