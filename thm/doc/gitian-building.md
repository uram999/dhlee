...
*Setup instructions for a Gitian build of THM Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the THM
...
- [Building THM Core](#building-thm-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-thm-host/README.md).
...
Clone the git repositories for thm and Gitian.
...
in the thm repository under 'Fetch and create inputs' to install sources which require
...
To build THM Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the thm repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/thm/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/thm/thm
...
For example:
```bash
URL=https://github.com/laanwj/thm.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit thm=${COMMIT} --url thm=${URL} ../thm/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit thm=${COMMIT} --url thm=${URL} ../thm/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit thm=${COMMIT} --url thm=${URL} ../thm/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the thm git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../thm/contrib/gitian-descriptors/*|sort|uniq )
...
THMPATH=/some/root/path/thm
SIGPATH=/some/root/path/thm-detached-sigs

./bin/gbuild --url thm=${THMPATH},signature=${SIGPATH} ../thm/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/thm-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/thm-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/thm-osx-build.assert
```
...
[thm-core/gitian.sigs](https://github.com/thm-core/gitian.sigs/) repository, or if that's not possible create a pull
