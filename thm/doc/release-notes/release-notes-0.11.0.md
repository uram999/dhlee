THM Core version 0.11.0 is now available from:

  <https://thm.org/bin/thm-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer THM/kB fee to be
...
For example, add the following to `thm.conf`:
...
related to the blockchain in the thm system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows THM Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that THM Core will 
...
For an example script see [thm-submittx](https://github.com/laanwj/thm-submittx).
...
- #5707 `6af674e` Change hathmoded character constants to descriptive named constants for db keys
...
- #6333 `41bbc85` Hathmoded seeds update June 2015
...
- #5334 `cf87536` libthmconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new thm-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only THMAmountField
- #5753 `0f44672` Add thm logo to about screen
...
- #5809 `46bfbe7` Add thm-cli man page
...
- #5849 `41259ca` contrib/init/thmd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename THM-Qt.app to THM-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for thm-tx)
