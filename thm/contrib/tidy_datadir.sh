#!/bin/bash
# Copyright (c) 2013 The THM Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete THM database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no THM datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old THM datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected THM 0.7 datadir."
    ;;
  3)
    echo "Detected THM pre-0.8 datadir."
    ;;
  4)
    echo "Detected THM 0.8 datadir."
    ;;
esac
