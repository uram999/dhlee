...
The RPM spec file provided here is for THM-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://thm.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[THM git repository](https://github.com/thm/thm) but are not part of
...
Currently only `Source10` is used, for the example `thm.conf` file.
...
in the thm git repository. At present only an SVG version of the THM
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the thm
...
needed when 0.12.1 is released, a proper fix is already in the THM git
master branch.
...
idea to build THM Core against a static version of that library compiled
...
The `thm.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `thm-server` package. At this point, most RPM based
...
The `thm-server` package should have SELinux support. How to properly do
...
THM-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/thm/). He has been
packaging THM for Fedora at least since 2012.
...
4. I renamed the `thm` package that contains the Qt GUI to `thm-core` as
that appears to be how the general population refers to it, in contrast to
`thm-xt` or `thm-classic`. I wanted to make sure the general population
