...
###   thmcore.org and thm.org and compares them.
...
WORKINGDIR="/tmp/thm_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://thmcore.org"
HOST2="https://thm.org"
BASEDIR="/bin/"
VERSIONPREFIX="thm-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'thm-',
...
if [ $? -ne 0 ]; then
   echo "thm.org failed to provide signature file, but thmcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "thm.org and thmcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the THM Core binary release signing key installed?"
   fi
